...
var UniversityService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.UniversityService = void 0;
const common_1 = require("@nestjs/common");
const uuid_1 = require("uuid");
const prisma_service_1 = require("../../database/prisma.service");
const chat_click_service_1 = require("./services/chat-click.service");
const email_service_1 = require("./services/email.service");
let UniversityService = UniversityService_1 = class UniversityService {
    constructor(prisma, chatClickService, emailService) {
        this.prisma = prisma;
        this.chatClickService = chatClickService;
        this.emailService = emailService;
        this.logger = new common_1.Logger(UniversityService_1.name);
        this.widgetStore = new Map();
        this.iframeLoads = new Map();
    }
    async generateWidget(config, userId) {
        let widgetId;
    ...
    async getUserWidget(userId) {
        try {
            ...
            if (profile.widgetConfig) {
                this.widgetStore.set(profile.widgetId, profile.widgetConfig);
            }
    ...
    async verifyIframeIntegration(widgetId, websiteUrl) {
        try {
            const loads = this.iframeLoads.get(widgetId);
            ...
            const matchingLoads = loads.filter(load => load.domain === targetDomain ||
    ...
    async getChatClickAnalytics(widgetId, userId) {
        if (userId) {
    ...
    async getChatClickCount(widgetId, userId) {
        if (userId) {
            const profile = await this.prisma.universityProfile.findFirst({
                where: {
                    widgetId,
    ...
    async saveChatClickAnswers(data) {
        try {
            ...
            if (data.ambassadorId && !chatClick.ambassadorId) {
                updateData.ambassadorId = data.ambassadorId;
            }
            if (data.ambassadorName && !chatClick.ambassadorName) {
    ...
    async getChatClicksByCountry(widgetId, userId) {
        if (userId) {
            const profile = await this.prisma.universityProfile.findFirst({
                where: {
                    widgetId,
    ...
    generateIframeFormats(widgetId) {
...
<iframe 
  ...
  title="University Navigation Widget">
    ...
    generateWidgetHTML(config, widgetId) {
...
<body>
  ...
  <script>
    ...
    async setUniversityEmail(userId, emailData) {
        try {
    ...
    async getUniversityEmail(userId) {
        try {
            const universityProfile = await this.prisma.universityProfile.findUnique({
                where: { userId },
                select: { email: true }
    ...
    async sendAmbassadorInvitation(userId, invitationData) {
        try {
            const universityProfile = await this.prisma.universityProfile.findUnique({
                where: { userId },
                select: {
            ...
            const result = await this.emailService.sendAmbassadorInvitation(invitationData, universityProfile.email, universityName);
            if (result.success) {
    ...
    async getInvitedAmbassadors(userId) {
        try {
    ...
    async updateInvitationStatus(ambassadorEmail, status) {
        try {
    ...
    async sendBulkAmbassadorInvitations(userId, bulkData) {
        try {
            const universityProfile = await this.prisma.universityProfile.findUnique({
                where: { userId },
                select: {
            ...
            for (const ambassador of bulkData.ambassadors) {
                try {
                    ...
                    const emailResult = await this.emailService.sendAmbassadorInvitation({
                        ...
                        universityName
                    }, universityProfile.email, universityName);
...
exports.UniversityService = UniversityService = UniversityService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        chat_click_service_1.ChatClickService,
